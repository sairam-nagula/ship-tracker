...
export async function fetchParadiseFromMTN(): Promise<ShipLocation> {
...
export async function GET() {
  try {
    const shipLocation = await fetchParadiseFromMTN();
    return NextResponse.json(shipLocation, { status: 200 });
  } catch (error: any) {
    console.error("Error in /api/Paradise/paradise-ship-location:", error);
