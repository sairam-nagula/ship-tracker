...
const MAX_HISTORY_HOURS = 24 * 11; // 1 days
const FALLBACK_HISTORY_HOURS = 18;
...
// Use whichever itinerary route corresponds to this ship.
// If this file is for Paradise, keep paradise-itinerary.
async function getDynamicHistoryHours(req: Request): Promise<number> {
  try {
    ...
    const url = `${proto}://${host}/api/Paradise/paradise-itinerary`;
...
async function fetchTrackFromMTN(historyHours: number): Promise<ShipTrackPoint[]> {
...
export async function GET(req: Request) {
  try {
    ...
    const track = await fetchTrackFromMTN(historyHours);
    return NextResponse.json(track, { status: 200 });
  } catch (error: any) {
    console.error("Error in /api/Islander/islander-trail:", error);
    return NextResponse.json(
      {
        error: "Failed to get ship track",
